...
class App {
  constructor(container, { items, bend, textColor = '#ffffff', borderRadius = 0, font = 'bold 30px Figtree', scrollSpeed = 2, scrollEase = 0.05, onLoopComplete } = {}) {
  ...
  createMedias(items, bend = 1, textColor, borderRadius, font) {
    ...
    this._uniqueCount = galleryItems.length;
  ...
  onCheck() {
    ...
    // Detect one full loop completion
    if (!this._loopFired && this.medias[0]) {
      const loopWidth = this.medias[0].widthTotal / 2; // half of doubled list = one loop
      if (Math.abs(this.scroll.target) >= loopWidth) {
        this._loopFired = true;
        if (this.onLoopComplete) this.onLoopComplete();
      }
    }
  }
  onResize() {
...
export default function CircularGallery({
  ...
  }, [items, bend, textColor, borderRadius, font, fontUrl, scrollSpeed, scrollEase, onLoopComplete]);
